...
#
#   ./scripts/db-down.sh           # stop and remove the container, keep the data
#   ./scripts/db-down.sh --purge   # also delete the volume, so db-up starts empty
set -uo pipefail
cd "$(dirname "$0")/.."
...
echo
if [ "${1:-}" = "--purge" ]; then
  echo "POSTGRES DOWN, volume deleted"
else
  echo "POSTGRES DOWN, volume hive-sandbox-pgdata kept (--purge deletes it)"
fi
